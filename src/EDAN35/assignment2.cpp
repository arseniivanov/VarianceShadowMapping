...
edan35::Assignment2::run()
{
	...
	mCamera.mMovementSpeed = 0.5f * constant::scale_lengths; // 3 m/s => 10.8 km/h.
    ...
    GLuint gaussian_blur_shader = 0u;
	program_manager.CreateAndRegisterProgram("Blur Shadow Map",
	                                         { { ShaderType::vertex, "EDAN35/gaussianBlur.vert" },
	                                           { ShaderType::fragment, "EDAN35/gaussianBlur.frag" } },
	                                         gaussian_blur_shader);
	if (gaussian_blur_shader == 0u) {
		LogError("Failed to load gaussian blur pass");
		return;
	}
    GLuint gaussian_blur_shader2 = 0u;
	program_manager.CreateAndRegisterProgram("Blur Shadow Map",
	                                         { { ShaderType::vertex, "EDAN35/gaussianBlur2.vert" },
	                                           { ShaderType::fragment, "EDAN35/gaussianBlur2.frag" } },
	                                         gaussian_blur_shader2);
	if (gaussian_blur_shader2 == 0u) {
		LogError("Failed to load gaussian blur pass 2");
		return;
	}
    ...
    auto const moments_texture                     = bonobo::createTexture(constant::shadowmap_res_x, constant::shadowmap_res_y, GL_TEXTURE_2D, GL_RG32F, GL_RG, GL_FLOAT, nullptr);
    auto const blur_texture                        = bonobo::createTexture(constant::shadowmap_res_x, constant::shadowmap_res_y, GL_TEXTURE_2D, GL_RG32F, GL_RG, GL_FLOAT, nullptr);
	...
	auto const shadowmap_fbo = bonobo::createFBO({moments_texture}, shadowmap_texture);
    auto const blur_fbo      = bonobo::createFBO({blur_texture}, shadowmap_texture);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if (!shader_reload_failed) {
			...
			// XXX: Is any other clearing needed? (Added color + depth buffer bit to remove sky-artifact)
			...
			// XXX: Is any clearing needed? (Added buffer bit clear since color buffer filled with previous info);
			for (size_t i = 0; i < static_cast<size_t>(lights_nb); ++i) {
				auto& lightTransform = lightTransforms[i];
				lightTransform.SetRotate(seconds_nb * 0.01f + i * 1.57f, glm::vec3(0.0f, 1.0f, 0.0f));
                ...
                GLenum shadow_draw_buffers[1] = {GL_COLOR_ATTACHMENT0}; //Add moments buffer to shadow 
                glDrawBuffers(1, shadow_draw_buffers);
				glViewport(0, 0, constant::shadowmap_res_x, constant::shadowmap_res_y);
				// XXX: Is any clearing needed? Yes we need to clear the depth buffer for every light.
                glClear(GL_DEPTH_BUFFER_BIT | GL_COLOR_BUFFER_BIT);
				...
				//
				//Pass 2.2: Blur moments (horizontal) for light i
				//
				if (gaussian_blur) {
                glCullFace(GL_BACK);
                glDepthFunc(GL_ALWAYS);
				glBindFramebuffer(GL_FRAMEBUFFER, blur_fbo);
                glUseProgram(gaussian_blur_shader);
                GLenum blur_draw_buffers[1] = {GL_COLOR_ATTACHMENT0}; //Add intermediate blur texture
                glDrawBuffers(1, blur_draw_buffers);
				glViewport(0, 0, constant::shadowmap_res_x, constant::shadowmap_res_y);
                
                bind_texture_with_sampler(GL_TEXTURE_2D, 0, gaussian_blur_shader, "moments_texture", moments_texture, default_sampler);
                
				GLStateInspection::CaptureSnapshot("Shadow Map Blur");
                
                bonobo::drawFullscreen();
                glBindSampler(0u, 0u);
                glUseProgram(0u);
                //
				//Pass 2.2: Blur moments (vertical) for light i
				//
                glClear(GL_DEPTH_BUFFER_BIT);
                glCullFace(GL_BACK);
                glDepthFunc(GL_ALWAYS);
				glBindFramebuffer(GL_FRAMEBUFFER, shadowmap_fbo);
                glUseProgram(gaussian_blur_shader2);
                GLenum blur_draw_buffers2[1] = {GL_COLOR_ATTACHMENT0}; //Write back blurred texture to moments.
                glDrawBuffers(1, blur_draw_buffers2);
 				glViewport(0, 0, constant::shadowmap_res_x, constant::shadowmap_res_y);

                bind_texture_with_sampler(GL_TEXTURE_2D, 0, gaussian_blur_shader2, "blur_texture", blur_texture, default_sampler);
 				GLStateInspection::CaptureSnapshot("Shadow Map Blur");
                ...
                glCullFace(GL_FRONT); // <-- this shit is important 
                }
				glEnable(GL_BLEND);
				...
				glViewport(0, 0, framebuffer_width, framebuffer_height);
				// XXX: Is any clearing needed? I dont think so, clearing color removes all other lights, and shadow buffer is same for all lights.

				auto const spotlight_set_uniforms = [framebuffer_width,framebuffer_height,this,&light_matrix,&lightColors,&lightTransform,&i,&use_variance,&use_offsets,&use_pcf,&reduce_bleeding](GLuint program){
                    glUniform1i(glGetUniformLocation(program, "bleeding"), reduce_bleeding);
                    glUniform1i(glGetUniformLocation(program, "vsm"), use_variance);
                    glUniform1i(glGetUniformLocation(program, "offsets"), use_offsets);
                    glUniform1i(glGetUniformLocation(program, "pcf"), use_pcf);
					glUniform2f(glGetUniformLocation(program, "inv_res"),
					            1.0f / (static_cast<float>(framebuffer_width)-0.5),
					            1.0f / (static_cast<float>(framebuffer_height)-0.5));
                ...
                bind_texture_with_sampler(GL_TEXTURE_2D, 3, accumulate_lights_shader, "moments_texture", moments_texture, shadow_sampler);
		...
		if (show_textures) {
            ...
            bonobo::displayTexture({ -0.45f,  0.55f}, {-0.05f,  0.95f}, blur_texture,                   default_sampler, {0, 1, 2, -1}, glm::uvec2(framebuffer_width, framebuffer_height), true, lightProjectionNearPlane, lightProjectionFarPlane);
            ...
            //bonobo::displayTexture({ 0.05f, 0.55f}, { 0.45f, 0.95f}, moments_texture, default_sampler, {0, 1, 2, -1}, glm::uvec2(framebuffer_width, framebuffer_height), true, mCamera.mNear, mCamera.mFar);
            bonobo::displayTexture({ 0.05f,  0.55f}, {0.45f,  0.95f}, moments_texture,                   default_sampler, {0, 1, 2, -1}, glm::uvec2(framebuffer_width, framebuffer_height), true, lightProjectionNearPlane, lightProjectionFarPlane);
		}
		//
		...
		if (opened) {
			...
			ImGui::Checkbox("Enable VSM", &use_variance);
			ImGui::Checkbox("Enable PCF", &use_pcf);
			ImGui::Checkbox("Enable Offsets", &use_offsets);
            ImGui::Checkbox("Reduce Light Bleed", &reduce_bleeding);
	...
	fallback_shader = 0u;
    glDeleteProgram(gaussian_blur_shader);
	gaussian_blur_shader = 0u;
}
